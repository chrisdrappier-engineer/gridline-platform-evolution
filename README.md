...
targets and deployment hardening. This runtime is separate from the development
Compose setup.
...
Manual provision of a fresh local production-like environment:
...
Manual deploy against an existing local production-like database:
...
Explicit evidence reset for local workload-lab preparation:
...
deployments. A later SaaS or multi-instance deployment should move uploads to
